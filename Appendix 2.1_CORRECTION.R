...
# I corrected a small mistake and improved the notations
...
occu_mcmc <- function(y, x, w, nmcmc) {
  ...
  for (k in 1:nmcmc) {
    ### Sample beta
    #--- Step 1. Propose new values using a Random Walk Metropolis-Hastings step
    # The proposal distribution is a multivariate normal centered at the current value
    # beta_tune adjusts the proposal variance to optimize the acceptance rate
    beta_star <- rnorm(ncovs + 1, beta, beta_tune)
    
    #--- Step 2. Evaluate the log-posterior for the proposed candidate (beta_star) and the current parameter values (beta)
    # Sum of the log-likelihood (Bernoulli state process) and log-prior (Normal priors)
    # inv.logit(cbind(1,x) %*% beta_star) maps the linear predictor to occupancy probability psi
    mh1 <- sum(dbinom(z, 1, inv.logit(cbind(1,x) %*% beta_star), log=TRUE)) + # log-likelihood
           sum(dnorm(beta_star, beta_mean, beta_sd, log=TRUE)) # Normal priors
    mh2 <- sum(dbinom(z, 1, inv.logit(cbind(1,x) %*% beta     ), log=TRUE)) + # log-likelihood
           sum(dnorm(beta     , beta_mean, beta_sd, log=TRUE)) # Normal priors
           
    #--- Step 3. Calculate the Metropolis acceptance ratio (mh)
    # We operate on the log scale to prevent numerical underflow, then exponentiate
    mh <- exp(mh1 - mh2)
    
    #--- Step 4. Stochastic acceptance/rejection decision
    # If the candidate increases the posterior density, it is always accepted (mh > 1)
    # If it decreases it, it is accepted with probability 'mh'
    if (mh > runif(1)) {
      beta <- beta_star
    }
    ...
    prob_occupy_prior <- inv.logit(cbind(1,x) %*% beta)

    #--- Step 2: Calculate the local observation likelihood ---
#   prob_observe_all_zeros <- apply((1 - p), 1, prod)
    prob_observe_all_zeros <- exp(rowSums(log(1 - p + 1e-15))) # this is a faster way of doing it

    #--- Step 3: Bayesian posterior calculation and sample ---

    # Weight for z=1: Prior * Local Likelihood
    weight_z_is_1 <- prob_occupy_prior * prob_observe_all_zeros

    # Weight for z=0: (1 - Prior) * Local Likelihood (=1 since y must be 0)
    weight_z_is_0 <- (1 - prob_occupy_prior) * 1
    ...
    sites_to_sample <- which(ysum == 0)
    z[sites_to_sample] <- rbinom(length(sites_to_sample), 1, prob_posterior_z1[sites_to_sample])

    # If detected (ysum > 0), the species must be present (z = 1)
    z[which(ysum > 0)] <- 1
...
par(mar=c(1,1,3,1))
plot(1, ylim=c(185, 205), type='n', axes=F, xlab='', ylab='')
...
axis(4, at=seq(185, 205, 5), cex.axis=1.6, las=2)
