#=================================================================================================
# Cormack-Jolly-Seber (CJS) model with data in individual capture history format
...
#=======================
cjs_indv_mcmc <- function(c, x, w, nmcmc) {
  ...
  for (k in 1:nmcmc) {
    ...
    for (t in 1:(nyear-1)) {
      prob1[t] <- sum(dbinom(z[which(f <= t),t+1], 1, z[which(f <= t),t] * phi_star[t], log=TRUE))
      prob2[t] <- sum(dbinom(z[which(f <= t),t+1], 1, z[which(f <= t),t] * phi     [t], log=TRUE))
    } # t
    mh1 <- sum(prob1) + 
    ...
    prob1 <- prob2 <- numeric(nyear-1)
    for (t in 1:(nyear-1)) {
      prob1[t] <- sum(dbinom(c[which(f <= t),t+1], 1, z[which(f <= t),t+1] * p_star[t], log=TRUE))
      prob2[t] <- sum(dbinom(c[which(f <= t),t+1], 1, z[which(f <= t),t+1] * p     [t], log=TRUE))
    } # t
    mh1 <- sum(prob1) + 
    ...
    for (v in 1:nindv) {
      if (l[v] <= nyear-2) {
        for (t in (l[v]+1):(nyear-1)) {
          if (z[v,t-1] == 1 & z[v,t+1] == 0) {
            num_temp <- phi[t-1] * (1 - phi[t]) * (1 - p[t-1])
            psi_temp <- num_temp / (num_temp + 1 - phi[t-1])
        ...
        if (z[v,nyear-1] == 1) {
          num_temp <- phi[nyear-1] * (1 - p[nyear-1])
          psi_temp <- num_temp / (num_temp + 1 - phi[nyear-1])
          z[v,nyear] <- rbinom(1, 1, psi_temp)
        }
        if (z[v,nyear-1] == 0) {
          z[v,nyear] <- 0
        }
...
out <- foreach (i=1:chain, .packages='boot') %dopar% {
  cjs_indv_mcmc(c=c, x=x, w=w, nmcmc=nmcmc)
} # i
...
save(output, file='4.1.1 cjs indv model_output.RData')

#==============
# Plot results
#==============
pdf(file='4.1.1.1 cjs indv model_chains.pdf', width=10, height=8)
...
pdf(file='4.1.1.2 cjs indv model_phi.pdf', width=10, height=8)

library(vioplot) # for making violin plots

for (i in 1:chain) {
  if (i == 1) {
    phi_post <- out[[i]]$phi[,(nmcmc*0.2+1):nmcmc]
  } else {
    phi_post <- cbind(phi_post, out[[i]]$phi[,(nmcmc*0.2+1):nmcmc])
  }
} # i
...
vioplot(t(phi_post), add=T, col='lightcoral', rectCol='grey36', lineCol='grey36', border=NA)
...
points(x=2.5, y=0.1, pch=16, cex=1, col='royalblue')
lines(x=c(2.1,2.9), y=c(0.1,0.1), lwd=1.2, col='royalblue')
vioplot(phi_post[1,]-median(phi_post[1,])+0.1, at=5.5, add=T, col='lightcoral', rectCol='grey36', lineCol='grey36', border=NA)
text(x=3, y=0.1, labels='True', pos=4, cex=1.8)
text(x=6, y=0.1, labels='Estimated', pos=4, cex=1.8)
