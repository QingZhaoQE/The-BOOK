...
diag(theta) <- 0 # no self-colonization
                 # this is redundant, as colonization happens only when z[i,t] = 0
                 # so, it is added here for readability
...
dist_dynam_occu_mcmc <- function(y, x, w, d, nmcmc) {
  ...
  for (t in 2:nyear) {
    gamma[,t-1] <- as.numeric(theta %*% z[,t-1])
  } # t
  ...
  for (k in 1:nmcmc) {
    ...
    theta_star <- eta_star / (rowSums(eta_star) + 1e-15)
    diag(theta_star) <- 0
...
#     prob_observe_all_zeros <- exp(rowSums(log(1 - p[,t,] + 1e-15)))
      ...
      for (i in sites_to_sample) {
        ...
        if (t < nyear) {
          ...
          if (length(target_sites) > 0) {
            gamma_z1 <- gamma[,t] - (theta[,i] * z[i,t]) + theta[,i] # colonization probability if z[i,t] = 1
            gamma_z0 <- gamma[,t] - (theta[,i] * z[i,t])             # colonization probability if z[i,t] = 0
      ...
      #--- Step 4: Bayesian posterior calculation and sample ---
        ...
        if (t < nyear & z[i,t] != old_zi) {
          ...
          gamma[,t] <- gamma[,t] + theta[,i] * (z[i,t] - old_zi)
        }
      } # i
