...
adj_dynam_occu_mcmc <- function(y, x, w, a, nmcmc) {
  ...
  for (k in 1:nmcmc) {
    ...
    for (t in 1:nyear) {
      ...
      # 1. Calculate Prior occupancy probability for the current year
      ...
      # 2. Calculate Observation Likelihood for the current year
      # Probability of observing all zeros given z=1: P(y_it=0 | z_it=1)
      # If ysum > 0, this value is irrelevant as z will be fixed to 1
      lik_y_vec <- exp(rowSums(log(1 - p[,t,] + 1e-15)))

      # 3. Identify sites where detection was zero (z must be sampled)
      z[ysum[,t] > 0, t] <- 1
      occ_neigh_count <- as.numeric(a %*% z[,t])
      sites_to_sample <- which(ysum[,t] == 0)
      if (length(sites_to_sample) > 1) {
        sites_to_sample <- sample(sites_to_sample)
      }
      ...
      for (i in sites_to_sample) {
        ...
        if (t < nyear) {
          ...
          gamma_it_if_z0 <- 1 - (1 - zeta) ^ (occ_neigh_count[i]) * (1 - delta)
          lik_self_future_z1 <- z[i,t+1] * phi[i,t] + (1 - z[i,t+1]) * (1 - phi[i,t])
          lik_self_future_z0 <- z[i,t+1] * gamma_it_if_z0 + (1 - z[i,t+1]) * (1 - gamma_it_if_z0)
          ...
          target_neighbors <- neighbors_i[z[neighbors_i, t] == 0]

          if (length(target_neighbors) > 0) {
            n_occ_others <- occ_neigh_count[target_neighbors] - old_zi
        
            prob_g_if_i1 <- 1 - (1 - zeta) ^ (n_occ_others + 1) * (1 - delta)
            prob_g_if_i0 <- 1 - (1 - zeta) ^ (n_occ_others) * (1 - delta)
        
            zj_next <- z[target_neighbors, t+1]
            lik_neigh_feedback_z1 <- prod(zj_next * prob_g_if_i1 + (1 - zj_next) * (1 - prob_g_if_i1))
            lik_neigh_feedback_z0 <- prod(zj_next * prob_g_if_i0 + (1 - zj_next) * (1 - prob_g_if_i0))
          } else {
        ...
        # 4. Integrate all components into posterior weights
        # Weight for z[i,t] = 1
        ...
        # 5. Draw the new state
        new_zi <- (runif(1) < (w1 / (w1 + w0 + 1e-12))) + 0

        if (new_zi != old_zi) {
          diff <- new_zi - old_zi
          occ_neigh_count[neighbors_i] <- occ_neigh_count[neighbors_i] + diff
          z[i,t] <- new_zi
        }
      } # i

      # 6. Immediate update of gamma_t for the next year's prior
