...
adj_dynam_occu_mcmc <- function(y, x, w, a, nmcmc) {
  ...
  for (k in 1:nmcmc) {
    ...
    for (t in 1:nyear) {
      ...
      # 1. Pre-calculate Prior and Observation Likelihood for the current year
      # This part only depends on z[, t-1], so it's constant for all i in the current year loop
      if (t == 1) {
        psi_prior_vec <- inv.logit(cbind(1, x[,1,]) %*% beta0)
      } else {
        # Note: gamma[, t-1] was already updated at the end of the (t-1) loop
        psi_prior_vec <- z[, t-1] * phi[, t-1] + (1 - z[, t-1]) * gamma[, t-1]
      }
      
      # Probability of observing all zeros given z=1: P(y_it=0 | z_it=1)
      # If ysum > 0, this value is irrelevant as z will be fixed to 1
      lik_y_vec <- apply(1 - p[,t,], 1, prod)

      # 2. Identify sites where detection was zero (z must be sampled)
      sites_to_sample <- which(ysum[, t] == 0)
      
      for (i in sites_to_sample) {
        
        # --- Future State Dependency (Markov Link to t+1) ---
        if (t < nyear) {
          ...
          lik_self_future_z0 <- z[i, t+1] * gamma[i, t] + (1 - z[i, t+1]) * (1 - gamma[i, t])
          ...
          if (length(neighbors) > 0) {
            for (j in neighbors) {
              # We only care about neighbors j that were empty at time t
              # because if z[j,t]=1, its next state depends on phi, not on i's source effect
              if (z[j, t] == 0) {
                # Count current occupied neighbors of j, excluding site i
                n_occ_others <- sum(a[j, -i] * z[-i, t])
                
                # Compare neighbor j's colonization outcome under two hypotheses for i
                prob_g_if_i_is_1 <- 1 - (1 - zeta)^(n_occ_others + 1) * (1 - delta)
                prob_g_if_i_is_0 <- 1 - (1 - zeta)^(n_occ_others) * (1 - delta)
                
                # Likelihood of observing z[j, t+1]
                lik_neigh_feedback_z1 <- lik_neigh_feedback_z1 * (z[j, t+1] * prob_g_if_i_is_1 + (1 - z[j, t+1]) * (1 - prob_g_if_i_is_1))
                lik_neigh_feedback_z0 <- lik_neigh_feedback_z0 * (z[j, t+1] * prob_g_if_i_is_0 + (1 - z[j, t+1]) * (1 - prob_g_if_i_is_0))
              }
            }
          }
        } else {
        ...
        # 3. Integrate all components into posterior weights
        # Weight for z[i,t] = 1
        w1 <- psi_prior_vec[i] * lik_y_vec[i] * lik_self_future_z1 * lik_neigh_feedback_z1
        # Weight for z[i,t] = 0 (Likelihood is 1 because ysum must be 0)
        w0 <- (1 - psi_prior_vec[i]) * 1 * lik_self_future_z0 * lik_neigh_feedback_z0
        
        # 4. Draw the new state
        post_prob <- w1 / (w1 + w0 + 1e-12)
        z[i, t] <- rbinom(1, 1, post_prob)
        
      } # i

      # 5. Fix z=1 for sites where species was detected
      z[which(ysum[, t] > 0), t] <- 1

      # 6. CRITICAL: Immediate update of gamma_t for the next year's prior
