...
adj_dynam_occu_mcmc <- function(y, x, w, a, nmcmc) {
  ...
  for (k in 1:nmcmc) {
    ...
    gamma_star_zeta <- matrix(, nsite, nyear-1)
    for (t in 2:nyear) {
      neighbor_sum <- as.numeric(a %*% z[,t-1])
    ...
    if (mh > runif(1)) {
      zeta <- zeta_star
      gamma <- gamma_star_zeta
    }

    ### Sample delta
    delta_star <- inv.logit(rnorm(1, logit(delta), delta_tune))
    gamma_star_delta <- matrix(, nsite, nyear-1)
    for (t in 2:nyear) {
        neighbor_sum <- as.numeric(a %*% z[,t-1])
    ...
    ### Sample z (Corrected for spatial-temporal dependency)
    for (t in 1:nyear) {

      #--- Step 1: Calculate the prior occupancy probability for the current year ---
      # This part only depends on z[,t-1], so it's constant for all i in the current year loop

      if (t == 1) {
        prob_occupy_prior <- inv.logit(cbind(1, x[,1,]) %*% beta0)
      } else {
        # Note: gamma[,t-1] was already updated at the end of the (t-1) loop
        prob_occupy_prior <- z[,t-1] * phi[,t-1] + (1 - z[,t-1]) * gamma[,t-1]
      }


      #--- Step 2: Calculate the local observation likelihood ---

#     prob_observe_all_zeros <- exp(rowSums(log(1 - p[,t,] + 1e-15)))
      prob_observe_all_zeros <- exp(rowSums(log(pmax(1 - p[,t,], 1e-15)))) # more stable


      #--- Step 3: Calculate future state dependency (The Markovian link) ---

      sites_to_sample <- which(ysum[,t] == 0) # Identify sites where detection was zero (z must be sampled)
      if (length(sites_to_sample) > 1) {
        sites_to_sample <- sample(sites_to_sample) # random scan to prevent systematic bias
      }

      for (i in sites_to_sample) {
        neighbors_i <- which(a[i,] == 1) # neighbors of site i

        if (t < nyear) {
          # A. Self-influence: How z[i,t] affects z[i,t+1]
          # If z[i,t]=1, transition is via phi; if z[i,t]=0, via gamma
          lik_self_future_z1 <- z[i,t+1] * phi[i,t] + (1 - z[i,t+1]) * (1 - phi[i,t])
          gamma_i_latest <- 1 - (1 - zeta) ^ sum(z[neighbors_i,t]) * (1 - delta) # instantaneous update of gamma[i,t]
          lik_self_future_z0 <- z[i,t+1] * gamma_i_latest + (1 - z[i,t+1]) * (1 - gamma_i_latest)

          # B. Spatial feedback: How z[i,t] acts as a source for neighbors at t+1
          lik_neigh_feedback_z1 <- 1
          lik_neigh_feedback_z0 <- 1

          target_neighbors <- neighbors_i[z[neighbors_i, t] == 0] # only matters for unoccupied neighbors

          if (length(target_neighbors) > 0) {
            # For each unoccupied neighbor j, count its occupied neighbors excluding site i
            n_occ_others <- numeric(length(target_neighbors))
            for (j in seq_along(target_neighbors)) {
              n_occ_others[j] <- sum(a[target_neighbors[j],] * z[,t]) - z[i,t] # occupied neighbors of j, excluding i
            } # j

            # Colonization probability of j at t+1, under z[i,t]=1
            gamma_i1 <- 1 - (1 - zeta) ^ (n_occ_others + 1) * (1 - delta)
            # Colonization probability of j at t+1, under z[i,t]=0
            gamma_i0 <- 1 - (1 - zeta) ^ (n_occ_others    ) * (1 - delta)
        
            zj_next <- z[target_neighbors, t+1]
            # Likelihood of observed z[j,t+1], assuming z[i,t]=1
            lik_neigh_feedback_z1 <- prod(zj_next * gamma_i1 + (1 - zj_next) * (1 - gamma_i1))
            # Likelihood of observed z[j,t+1], assuming z[i,t]=0
            lik_neigh_feedback_z0 <- prod(zj_next * gamma_i0 + (1 - zj_next) * (1 - gamma_i0))
          } else {
            # No unoccupied neighbors: spatial feedback is uninformative
            lik_neigh_feedback_z1 <- lik_neigh_feedback_z0 <- 1
          }
        } else {
          # Terminal year: no future or feedback info available
          lik_self_future_z1 <- lik_self_future_z0 <- 1
          lik_neigh_feedback_z1 <- lik_neigh_feedback_z0 <- 1
        }
      ...
      #--- Step 4: Bayesian posterior calculation and sample ---
        ...
        # Weight for z[i,t] = 1
        weight_z_is_1 <- prob_occupy_prior[i] * prob_observe_all_zeros[i] * lik_self_future_z1 * lik_neigh_feedback_z1
        # Weight for z[i,t] = 0
        weight_z_is_0 <- (1 - prob_occupy_prior[i]) * 1 * lik_self_future_z0 * lik_neigh_feedback_z0
        # Normalize: calculate the posterior probability of z=1
        prob_posterior_z1 <- weight_z_is_1 / (weight_z_is_1 + weight_z_is_0)

        z[i,t] <- rbinom(1, 1, prob_posterior_z1)
      } # i


      #--- Step 5: Immediate update of gamma_t for the next year's prior ---
      # This ensures z[,t] influence is propagated correctly to z[,t+1]
      # Update gamma[,t] for all i's outside the i-loop is essentially a block update of gamma

      if (t < nyear) {
        gamma[,t] <- 1 - (1 - zeta) ^ (as.numeric(a %*% z[,t])) * (1 - delta)
      }
    } # t
