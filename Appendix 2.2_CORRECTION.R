...
# with corrections and improved notations in sampling z
...
dynam_occu_mcmc <- function(y, x, w, nmcmc) {
  ...
  for (k in 1:nmcmc) {
    ...
    ### Sample z (Corrected for temporal dependency)
    for (t in 1:nyear) {
      ...
      if (t == 1) {
        ...
        # Subsequent years: occupancy depends on the state of the previous year (z[t-1])
        # If z[t-1]=1, it is persistence (phi); if z[t-1]=0, it is colonization (gamma)
...
#     prob_observe_all_zeros <- apply(1 - p[,t,], 1, prod)
      ...
      if (t < nyear) {
        # Likelihood of observing z[t+1] given z[t]=1 (Persistence logic)
        # If z[t+1]=1, prob is phi; if z[t+1]=0, prob is 1-phi
        prob_future_if_occupied <- z[,t+1] * phi[,t] + (1 - z[,t+1]) * (1 - phi[,t])
  
        # Likelihood of observing z[t+1] given z[t]=0 (Colonization logic)
        # If z[t+1]=1, prob is gamma; if z[t+1]=0, prob is 1-gamma
        prob_future_if_empty    <- z[,t+1] * gamma[,t] + (1 - z[,t+1]) * (1 - gamma[,t])
      } else {
        # For the final year, there is no future state to consider
        # Setting weights to 1 ensures they don't affect the multiplication
        prob_future_if_occupied <- rep(1, nsite)
        prob_future_if_empty    <- rep(1, nsite)
      }
      ...
      # Weight for z=0: (1 - Prior) * Local Likelihood (=1 since y must be 0) * Future Constraint
      ...
      z[sites_to_sample, t] <- rbinom(length(sites_to_sample), 1, prob_posterior_z1[sites_to_sample])
