...
diag(theta) <- 0 # no self-colonization
...
dist_cov_dynam_occu_mcmc <- function(y, x, w, d, nmcmc) {
  ...
  for (k in 1:nmcmc) {
    ...
    ### Sample z (Corrected for Distance-decay Kernel)

    # Calculate the component of gamma that is influenced by environment
    # This part will not change when z change
    gamma_env <- gamma_space <- matrix(, nsite, nyear-1)
    for(t in 2:nyear) {
      gamma_env[,t-1] <- inv.logit(cbind(1, x[,t,]) %*% beta_gamma)
      gamma_space[,t-1] <- as.numeric(theta %*% z[,t-1])
    } # t
    ...
    for (t in 1:nyear) {
      ...
      prob_observe_all_zeros <- exp(rowSums(log(pmax(1 - p[,t,], 1e-15))))
      ...
      for (i in sites_to_sample) {
        ...
        old_zi <- z[i,t] # save the current status of z[i,t]
        ...
        if (t < nyear) {
          ...
          # B. Spatial feedback: How z[i,t] acts as a source for other sites at t+1

          target_sites <- which(z[,t] == 0 & (1:nsite) != i) # only matters for unoccupied sites

          if (length(target_sites) > 0) {
            # colonization probability if z[i,t] = 1
            gamma_z1 <- gamma_env[,t] * ( gamma_space[,t] - (theta[,i] * z[i,t]) + theta[,i] )
            # colonization probability if z[i,t] = 0
            gamma_z0 <- gamma_env[,t] * ( gamma_space[,t] - (theta[,i] * z[i,t]) )
            lik_feedback_z1 <- prod(dbinom(z[target_sites, t+1], 1, gamma_z1[target_sites], log=FALSE))
            lik_feedback_z0 <- prod(dbinom(z[target_sites, t+1], 1, gamma_z0[target_sites], log=FALSE))
          } else {
            # no colonized sites: spatial feedback is uninformative
            lik_feedback_z1 <- lik_feedback_z0 <- 1
          }
        } else {
      ...
      #--- Step 4: Bayesian posterior calculation and sample ---
        ...
        # Weight for z[i,t] = 1
        weight_z_is_1 <- prob_occupy_prior[i] * prob_observe_all_zeros[i] * lik_self_future_z1 * lik_feedback_z1
        # Weight for z[i,t] = 0
        weight_z_is_0 <- (1 - prob_occupy_prior[i]) * 1 * lik_self_future_z0 * lik_feedback_z0
        ...
        if (t < nyear & z[i,t] != old_zi) {
          # immediate update of gamma if z[i,t] changes
          gamma_space[,t] <- gamma_space[,t] + theta[,i] * (z[i,t] - old_zi)
          gamma[,t] <- gamma_env[,t] * gamma_space[,t]
        }
      } # i
